...
export function CartDrawer({ open, onOpenChange }: CartDrawerProps) {
    const { cart, updateQuantity, removeFromCart, cartCount, cartTotal, refetchCart, productsMap, variantMap } = useCart();
                                                    ...
                                                    <div className="flex flex-col flex-1 py-0.5 justify-between">
                                                        <div className="flex justify-between gap-2.5 items-start">
                                                            <div className="flex-1">
                                                                ...
                                                                {/* Variant display */}
                                                                {(item.selectedParentType || item.selectedChildType) && (
                                                                    <div className="text-xs text-slate-500 mt-1 font-medium leading-relaxed">
                                                                        <p>
                                                                            Variant: {item.selectedParentType?.name} {item.selectedChildType?.name ? `➔ ${item.selectedChildType.name}` : ''}
                                                                        </p>
                                                                    </div>
                                                                )}
                                                            </div>
